...
///
/// Раньше удаление шло одной попыткой прямо на старте — и почти всегда
/// проваливалось. Причина в гонке: обновлённый процесс запускается из
/// download_and_apply() мгновенно, а старый живёт после этого ещё ~400 мс
/// (пауза на отрисовку «Перезапуск...»). Новый успевал дойти сюда раньше,
/// чем старый умрёт, файл был ещё залочен Windows, ошибка глоталась через
/// `let _`, и Wanderlust.old.exe оставался лежать рядом с лаунчером до
/// следующего ручного запуска — тот самый мусор на рабочем столе.
///
/// Поэтому: несколько попыток с паузой, в фоновом потоке (старт окна ждать
/// этого не должен). Заодно подчищаем .new.exe — недокачанный хвост от
/// оборвавшегося обновления.
pub fn cleanup_old_binary() {
    ...
    std::thread::spawn(move || {
        for stale in [
            ...
            // ~3 секунды суммарно: старому процессу с запасом хватает выйти.
            for _ in 0..12 {
